...
(function () {
  ...
  window.houseTypes = ['palace', 'flat', 'house', 'bungalo'];
  ...
  var addAdvert = function () {
    for (var i = 0; i < window.adverts.length; i++) {
      ...
      offer: {
        ...
        address: '',
        price: window.util.randomNumber(500, 10000),
        type: window.houseTypes[window.util.randomNumber(0, window.houseTypes.length - 1)],
        rooms: window.util.randomNumber(1, 10),
        guests: window.util.randomNumber(1, 10),
        checkin: bookingTimes[window.util.randomNumber(0, bookingTimes.length - 1)],
        checkout: bookingTimes[window.util.randomNumber(0, bookingTimes.length - 1)],
        features: window.util.getRandomList(featuresList),
        description: 'строка с описанием',
        photos: window.util.getRandomList(photosList),
      },
      location: {
        x: window.util.randomNumber(MIN_MAP_WIDTH, MAX_MAP_WIDTH),
        y: window.util.randomNumber(MIN_MAP_HEIGHT, MAX_MAP_HEIGHT),
      }
      };
